...
namespace Tsonic.CSharp.Js.RegExpRuntime.Engine.QuickJs.Unicode;
...
internal static class UnicodeProperties
{
    ...
    private static void CrSortAndRemoveOverlap(List<uint> pts)
    {
        int cnt = pts.Count / 2;
        if (cnt <= 1) return;
        // Parallel arrays keep sorting allocation and comparison overhead explicit.
        ...
        System.Array.Sort(indices, (a, b) => starts[a].CompareTo(starts[b]));
        pts.Clear();
